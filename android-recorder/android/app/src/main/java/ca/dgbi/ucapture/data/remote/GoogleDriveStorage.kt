...
class GoogleDriveStorage @Inject constructor(
    ...
    override suspend fun upload(
        ...
        // Try upload, and if we get an auth error, refresh token and retry once
        val result = executeUpload(audioFile, metadataFile, targetFolder)
        if (result.error is UploadError.NotAuthenticated) {
            Log.d("GoogleDriveStorage", "Upload got auth error, refreshing token and retrying")
            if (authManager.refreshToken()) {
                return@withContext executeUpload(audioFile, metadataFile, targetFolder)
            }
        }
        result
    }
    ...
    private suspend fun executeUpload(
        ...
        try {
            ...
            Log.e("GoogleDriveStorage", "Upload failed: HTTP ${e.statusCode} - ${e.statusMessage}", e)
            return UploadResult(
                success = false,
                error = classifyHttpError(e)
            )
        } catch (e: IOException) {
            Log.e("GoogleDriveStorage", "Upload failed: network error", e)
            return UploadResult(
                success = false,
                error = UploadError.NetworkError
            )
        } catch (e: Exception) {
            Log.e("GoogleDriveStorage", "Upload failed: ${e.message}", e)
            return UploadResult(
                success = false,
    ...
    /**
     * Classify Google API HTTP errors into appropriate UploadError types.
     */
    private fun classifyHttpError(e: GoogleJsonResponseException): UploadError {
        return when (e.statusCode) {
            401 -> UploadError.NotAuthenticated
            403 -> {
                // 403 can be quota exceeded or permission denied
                val reason = e.details?.errors?.firstOrNull()?.reason
                when (reason) {
                    "storageQuotaExceeded", "userRateLimitExceeded",
                    "rateLimitExceeded", "dailyLimitExceeded" -> UploadError.QuotaExceeded
                    else -> UploadError.NotAuthenticated // permission issue, re-auth may help
                }
            }
            404 -> UploadError.NoTargetFolder // folder likely deleted
            else -> UploadError.ApiError(e.statusCode, e.statusMessage ?: "HTTP ${e.statusCode}")
        }
    }
    ...
    override suspend fun verifyUpload(fileId: String, expectedMd5: String): Boolean =
        withContext(Dispatchers.IO) {
            try {
                ...
                Log.e("GoogleDriveStorage", "verifyUpload failed for $fileId", e)
                false
            }
        }
